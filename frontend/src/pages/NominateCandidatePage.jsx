...
import { UserPlus, ArrowLeft, AlertCircle, CheckCircle2, Award, FileText, Image } from 'lucide-react';

export default function NominateCandidatePage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    async function loadElections() {
      try {
        ...
        setErrorMsg('Failed to load active elections for nomination.');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.manifesto.trim()) {
      setErrorMsg('Campaign manifesto is required.');
      return;
    }
    ...
    try {
      ...
      if (res?.success || res?.data) {
        setSuccessMsg('Candidate nomination submitted successfully! Pending Election Commission review.');
        setTimeout(() => {
          navigate('/candidates');
        }, 1200);
      }
    } catch (err) {
      setErrorMsg(err.response?.data?.message || 'Failed to submit candidate nomination.');
    } finally {
    ...
    <div className="max-w-2xl mx-auto space-y-6 py-4">
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-white tracking-tight">Apply for Candidate Nomination</h1>
            <p className="text-xs text-slate-400">Submit your candidacy manifesto for upcoming college elections</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-slate-100 text-sm placeholder-slate-600 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all"
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1.5 uppercase tracking-wider">
              Campaign Manifesto * (At least 10 characters)
            </label>
            <textarea
              name="manifesto"
              ...
              minLength={10}
              maxLength={5000}
              placeholder="Write your campaign goals, key policy promises, and why voters should choose you..."
              required
            ...
            <p className="text-[11px] text-slate-500 mt-1 text-right">
              {formData.manifesto.length}/5000 characters
            </p>
          </div>
