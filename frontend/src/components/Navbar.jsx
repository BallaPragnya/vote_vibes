    ...
    <header className="sticky top-0 z-50 bg-slate-950/85 backdrop-blur-xl border-b border-slate-800/80 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Desktop Role-Aware Navigation Links */}
          <nav className="hidden md:flex items-center gap-1">
            ...
            {/* Role: VOTER / All Authenticated Users */}
            {isAuthenticated && (
              <Link
                to="/voter/dashboard"
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  isActive('/voter/dashboard') || isActive('/dashboard')
                    ? 'bg-indigo-500/10 text-indigo-300 font-semibold border border-indigo-500/20'
                    : 'text-slate-300 hover:text-white hover:bg-slate-900/60'
                }`}
              >
            ...
            {/* Role: CANDIDATE */}
            {isAuthenticated && (role === 'CANDIDATE' || role === 'ADMIN') && (
              <Link
                to="/candidate/portal"
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
              ...
              <div className="relative">
                ...
                {userDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-56 rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl py-2 z-50 animate-fadeIn">
                    ...
                    <div className="py-1">
                      <Link
                        to={role === 'ADMIN' ? '/admin/dashboard' : role === 'CANDIDATE' ? '/candidate/portal' : '/voter/dashboard'}
